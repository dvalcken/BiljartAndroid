...
class SimplifiedPlayerRepositoryTest {
    ...
    fun setUp() {
        ...
        // Setup the API service to return a flow of API players
//        coEvery { mockApiService.getPlayerssAsFlow() } returns flowOf(FakePlayerDataSource.players)
        ...
        coEvery { mockApiService.getAllRanks() } returns FakePlayerDataSource.players

        // Mock DAO insert operation, which returns Unit (= void)
    ...
    fun `refreshPlayers should fetch from API and insert into DAO`() = runTest {
        ...
        coVerify(exactly = FakePlayerDataSource.players.size) { mockPlayerDao.insert(any()) }
    }

    @Test
    fun `getById should return player from DAO`() = runTest {
        val playerId = FakePlayerDataSource.players.first().player_id
        every { mockPlayerDao.getById(playerId) } returns flowOf(FakePlayerDataSource.players.first().asDomainObject().asDbPlayer())

        val result = repository.getById(playerId).toList().first()

        // Verify that the player fetched from DAO is correct
        assertEquals(FakePlayerDataSource.players.first().name, result.name)
    }

    @Test
    fun `insert should insert player into DAO`() = runTest {
        val newPlayer = FakePlayerDataSource.players.first().asDomainObject()
        repository.insert(newPlayer)

        // Verify that the DAO insert method was called with the correct player
